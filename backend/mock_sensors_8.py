...
def build_reading(sensor: SensorStation, sequence: int) -> dict[str, float | str]:
    ...
    if spike_window and sensor["sensor_id"] in ("park_south", "downtown_center"):
        so2 = 185.0
        pm25 = 65.0
...
async def run_mock_sensors() -> None:
    ...
    client = MQTTClient("plumetrace-mock-sensors-8")
    if mqtt_user:
    ...
    try:
        ...
        logger.info("Mock 8-sensors connected to %s:%d", MQTT_BROKER_HOST, MQTT_BROKER_PORT)
